...
const onSubmitModal = async () => {
  if (stateModal.value == "add") {
    ...
    // }
    const body = {
      ServiceID: formModal.value.ServiceID,
      ServiceCode: formModal.value.ServiceCode,
      ServiceName: formModal.value.ServiceName,
    ...
    console.log(body)
    await _apiService.update(body, (response) => {
      if (response.Status === true) {
        document.getElementById("modal-manager").close();
        onLoadData();
      } else {
