...
import _apiEmployee from "../../../../api/employee.js";
import modalConfirm from "../../../../components/modal-confirm.vue";
import _apiDDL from "../../../../api/ddl.js";
import { getTokenStorage } from "../../../../helpers/storage.js";
import { useStore } from "vuex";
const columns = [
  { label: "รหัสพนักงาน", size: "lg", width: "25%" },
  { label: "ชื่อพนักงาน", size: "lg", width: "25%" },
  { label: "E-Mail", size: "lg", width: "25%" },
...
const onSubmitModal = async () => {
  if (stateModal.value == "add") {
    const body = {
      ...
      RoleID: formModal.value.RoleID,
      Department: formModal.value.Department,
      Email: formModal.value.Email,
      UserName: formModal.value.UserName,
      Password: formModal.value.Password,
      Remark: formModal.value.Remark,
    };
    await _apiEmployee.create(body, (response) => {
    ...
    console.log(body)
    await _apiEmployee.update(body, (response) => {
      if (response.Status === true) {
        document.getElementById("modal-manager").close();
        onLoadData();
      } else {
  ...
  <div class="flex justify-end space-x-2">
    ...
    <button class="rounded-full px-8 py-2 bg-custom-blue hover:bg-amber-600t" @click="onOpenModal('add')">
      <div class="flex items-center space-x-2">
        ...
        <span class="text-white font-bold">เพิ่มข้อมูลพนักงาน</span>
      </div>
    </button>
  </div>
  <!-- ---------------------------------------------------------------------------------------------------------------- -->
    ...
    <div class="modal-box max-w-4xl px-8">
      <div class="flex space-x-3">
        ...
        <span class="font-bold text-xl">{{
          stateModal == "add" ? "เพิ่มข้อมูลพนักงาน" : "แก้ไขข้อมูลพนักงาน"
        }}</span>
      </div>
        ...
        <div class="mt-3 mb-3">
          <div class="flex flex-wrap flex-row">
            <div class="px-3 basis-1/5 mb-3">
              <label class="label font-bold">รหัสพนักงาน <span style="color: red">*</span></label>
              <input required v-model="formModal.EmpCode" placeholder="รหัสพนักงาน" type="text"
                class="outline h-10 rounded-lg px-3 outline-gray-300 outline-2 w-full" />
            </div>
            <div class="px-3 basis-2/5 mb-3">
              <label class="label font-bold">ชื่อพนักงาน <span style="color: red">*</span></label>
              <input required v-model="formModal.Name" placeholder="ชื่อพนักงาน" type="text"
                class="outline h-10 rounded-lg px-3 outline-gray-300 outline-2 w-full" />
            </div>
            <div class="px-3 basis-2/5 mb-3">
