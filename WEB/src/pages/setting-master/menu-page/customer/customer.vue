...
const columns = [
  { label: "รหัสลูกค้า", size: "lg", width: "15%" },
  { label: "ชื่อลูกค้า", size: "lg", width: "25%" },
  { label: "เบอร์ติดต่อ", size: "lg", width: "25%" },
...
const onSubmitModal = async () => {
  if (stateModal.value == "add") {
    ...
    const body = {
      CusID: formModal.value.CusID,
      CusCode: formModal.value.CusCode,
    ...
    console.log(body)
    await _apiCustomer.update(body, (response) => {
      if (response.Status === true) {
        document.getElementById("modal-manager").close();
        onLoadData();
      } else {
...
const onEmitValueConfirm = () => {
  const body = { CusID: rowDelete.value.row.CusID };
  _apiCustomer.delete(body, (response) => {
...
watch(
  ...
  async (val) => {
    rows.value = rowsOriginal.value.filter((item) => {
      return (
        item.CusCode?.includes(val) ||
        item.Name?.includes(val) ||
        item.Surname?.includes(val)
      );
    });
  }
);
  ...
  <div class="flex justify-end space-x-2">
    ...
    <button class="rounded-full px-8 py-2 bg-custom-blue hover:bg-amber-600t" @click="onOpenModal('add')">
      <div class="flex items-center space-x-2">
        ...
        <span class="text-white font-bold">เพิ่มข้อมูลลูกค้า</span>
      </div>
    </button>
  </div>
  <!-- ---------------------------------------------------------------------------------------------------------------- -->
    ...
    <table class="table rounded-xl w-full mt-3">
      ...
      <tbody>
        <tr v-for="(row, index) in rows" :key="index">
          ...
          <td>
            <span class="font-bold">{{ row?.Telephone || "" }}</span>
          </td>
          <td>
    ...
    <div class="modal-box max-w-4xl px-8">
      <div class="flex space-x-3">
        ...
        <span class="font-bold text-xl">{{
          stateModal == "add" ? "เพิ่มข้อมูลลูกค้า" : "แก้ไขข้อมูลลูกค้า"
        }}</span>
      </div>
        ...
        <div class="mt-3 mb-3">
          <div class="flex flex-wrap flex-row">
            <div class="px-3 basis-1/5 mb-3">
              <label class="label font-bold">รหัสลูกค้า <span style="color: red">*</span></label>
              <input required v-model="formModal.CusCode" placeholder="รหัสลูกค้า" type="text"
                class="outline h-10 rounded-lg px-3 outline-gray-300 outline-2 w-full" />
            </div>
            <div class="px-3 basis-2/5 mb-3">
              <label class="label font-bold">ชื่อลูกค้า <span style="color: red">*</span></label>
              <input required v-model="formModal.Name" placeholder="ชื่อลูกค้า" type="text"
                class="outline h-10 rounded-lg px-3 outline-gray-300 outline-2 w-full" />
            </div>
            <div class="px-3 basis-2/5 mb-3">
              <label class="label font-bold">นามสกุล <span style="color: red">*</span></label>
              <input required v-model="formModal.Surname" placeholder="นามสกุล" type="text"
                class="outline h-10 rounded-lg px-3 outline-gray-300 outline-2 w-full" />
            </div>
            ...
            <div class="px-3 basis-2/6 mb-3">
              ...
              <select required class="outline h-10 rounded-lg px-3 outline-gray-300 outline-2 w-full"
                ...
                <option v-for="items in DDLBarch" :key="items.Code" :value="items.Code">
                  {{ items.Name }}
                </option>
              </select>
