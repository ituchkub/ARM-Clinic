...
const onSubmitModal = async () => {
  if (stateModal.value == "add") {
    ...
    const body = {
      ...
      ProductCode: formModal.value.ProductCode,
      ProductName: formModal.value.ProductName,
    ...
    console.log(body)
    await _apiProduct.update(body, (response) => {
      if (response.Status === true) {
        document.getElementById("modal-manager").close();
        onLoadData();
      } else {
  ...
  <div class="flex justify-end space-x-2">
    ...
    <button class="rounded-full px-8 py-2 bg-custom-blue hover:bg-amber-600t" @click="onOpenModal('add')">
      <div class="flex items-center space-x-2">
        ...
        <span class="text-white font-bold">เพิ่มข้อมูลสินค้า</span>
      </div>
    </button>
  </div>
  <!-- ---------------------------------------------------------------------------------------------------------------- -->
    ...
    <div class="modal-box max-w-4xl px-8">
      <div class="flex space-x-3">
        ...
        <span class="font-bold text-xl">{{
          stateModal == "add" ? "เพิ่มข้อมูลสินค้า" : "แก้ไขข้อมูลสินค้า"
        }}</span>
      </div>
