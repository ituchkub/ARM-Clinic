...
import _apiProductStock from "../../../../api/productStock.js";
import _apiDDL from "../../../../api/ddl.js";
...
const onSubmitModal = async () => {
  if (stateModal.value == "add") {
    ...
    // }
    const body = {
      ProductStockID: formModal.value.ProductStockID,
      ProductID: formModal.value.ProductID,
      Amount: formModal.value.Amount,
      StockDate: formModal.value.StockDate,
    };
    await _apiProductStock.create(body, (response) => {
    ...
    console.log(body)
    await _apiProductStock.update(body, (response) => {
      if (response.Status === true) {
        document.getElementById("modal-manager").close();
        onLoadData();
      } else {
...
const onLoadData = async () => {
  ...
  await _apiDDL.DDLProductList(
    {},
    (response) => {
      DDLProduct.value = response.body;
    }
  )


  await _apiProductStock.getList(
...
const onEmitValueConfirm = () => {
  const body = { ProductStockID: rowDelete.value.row.ProductStockID };
  _apiProductStock.delete(body, (response) => {
  ...
  <div class="flex justify-end space-x-2">
    ...
    <button class="rounded-full px-8 py-2 bg-custom-blue hover:bg-amber-600t" @click="onOpenModal('add')">
      <div class="flex items-center space-x-2">
        ...
        <span class="text-white font-bold">เพิ่มข้อมูลสต๊อกสินค้า</span>
      </div>
    </button>
  </div>
  <!-- ---------------------------------------------------------------------------------------------------------------- -->
    ...
    <table class="table rounded-xl w-full mt-3">
      ...
      <tbody>
        <tr v-for="(row, index) in rows" :key="index">
          ...
          <td>
            <span class="font-bold">{{ row?.StockDate || "" }}</span>
          </td>
          <td>
    ...
    <div class="modal-box max-w-4xl px-8">
      <div class="flex space-x-3">
        ...
        <span class="font-bold text-xl">{{
          stateModal == "add" ? "เพิ่มข้อมูลสต๊อกสินค้า" : "แก้ไขข้อมูลสต๊อกสินค้า"
        }}</span>
      </div>
          ...
          <div class="flex flex-wrap flex-row">
            ...
            <div class="px-3 basis-2/5 mb-3">
              ...
              <select required class="outline h-10 rounded-lg px-3 outline-gray-300 outline-2 w-full"
                v-model="formModal.ProductID">
                <option value="0" disabled selected hidden>เลือกสินค้า</option>
                <option v-for="items in DDLProduct" :key="items.Code" :value="items.Code">
                  {{ items.Name }}
                </option>
              </select>
            </div>
            <div class="px-3 basis-1/5 mb-3">
              <label class="label font-bold">จำนวน <span style="color: red">*</span></label>
              <input required v-model="formModal.Amount" placeholder="จำนวน" type="text"
                class="outline h-10 rounded-lg px-3 outline-gray-300 outline-2 w-full" />
            </div>
            <div class="px-3 basis-2/5 mb-3">
              <label class="label font-bold">วันที่ <span style="color: red">*</span></label>
              <input required v-model="formModal.StockDate" placeholder="วันที่" type="date"
                class="outline h-10 rounded-lg px-3 outline-gray-300 outline-2 w-full" />
            ...
            <!-- <div class="px-3 basis-2/5 mb-3">
              ...
              <input required v-model="formModal.ProductDetail" placeholder="วันที่เปิดทำการ" type="text"
                class="outline h-10 rounded-lg px-3 outline-gray-300 outline-2 w-full" />
            </div> -->
